...
func newFile(z Zengine, relpath string) *File {
	...
	f.µ = z
	f.RelPath = relpath
	f.DirRel = filepath.Dir(f.RelPath)
	f.FullPath = normalizeFilePath(filepath.Join(Ctx.SrcDir, relpath))
	f.DirFull = filepath.Dir(f.FullPath)
	return &f
}

func normalizeFilePath(fpath string) string {
	if runtime.GOOS == "windows" {
		return strings.ToLower(filepath.FromSlash(fpath))
	}
	return fpath
}

func onFilesClosed(µ Zengine, relpaths []string) {
	for i, _ := range relpaths {
		relpaths[i] = normalizeFilePath(relpaths[i])
	}
...
func onFilesOpened(µ Zengine, relpaths []string) {
	for _, relpath := range relpaths {
		...
		if isopened := !uslice.StrHas(OpenFiles, relpath); isopened {
