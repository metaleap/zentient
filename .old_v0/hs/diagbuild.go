...
func (_ *zhs) BuildFrom(filerelpaths []string) (freshdiags map[string]udev.SrcMsgs) {
	...
	filefullpath := filepath.Join(srcDir, filerelpath)
	dirfullpath := filepath.Dir(filefullpath)
	cmdargs := append(append([]string{"build"}, udevhs.StackArgs...), udevhs.StackArgsBuild...)
	raw, _ := urun.CmdExecIn(dirfullpath, "stack", cmdargs...)
	lns := ustr.Split(raw, "\n")
	...
	_wl := len(_w)
	for i := 0; i < len(lns); i++ {
		if ustr.Pref(lns[i], _w) && strings.Index(lns[i], "stack.yaml: ") >= _wl {
			lnstackwarns = append(lnstackwarns, strings.TrimLeft(lns[i][_wl:], "/\\"))
			lns = append(lns[:i], lns[i+1:]...)
			i--
		}
	}

	for len(lns) > 0 && len(lns[0]) == 0 {
		lns = lns[1:]
	}
	wasnoop := len(lns) == 0
	...
	if !wasnoop {
		...
		addlastcur := func() {
			if cur != nil {
				...
				if ustr.Suff(fpath, ".hs") {
					cur.Misc = "ghc"
				}
				cur.Ref = "stack"
				cur.Msg = strings.TrimSpace(cur.Msg)
				if i := strings.Index(cur.Msg, "]"); ustr.Pref(cur.Msg, "[") && i > 0 {
					cur.Ref = cur.Ref + "  » " + cur.Msg[1:i]
					cur.Msg = strings.TrimSpace(cur.Msg[i+1:])
				}
