...
// Called by `HandleRequest` after unmarshaling the given `CompletionsRequest` to populate the given `CompletionsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnCompletionsRequest func(*CompletionsRequest, *CompletionsResponse)error

// Called by `HandleRequest` after unmarshaling the given `NextRequest` to populate the given `NextResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnNextRequest func(*NextRequest, *NextResponse)error

// Called by `HandleRequest` after unmarshaling the given `SetExceptionBreakpointsRequest` to populate the given `SetExceptionBreakpointsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnSetExceptionBreakpointsRequest func(*SetExceptionBreakpointsRequest, *SetExceptionBreakpointsResponse)error

// Called by `HandleRequest` after unmarshaling the given `ConfigurationDoneRequest` to populate the given `ConfigurationDoneResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnConfigurationDoneRequest func(*ConfigurationDoneRequest, *ConfigurationDoneResponse)error

// Called by `HandleRequest` after unmarshaling the given `InitializeRequest` to populate the given `InitializeResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnInitializeRequest func(*InitializeRequest, *InitializeResponse)error

// Called by `HandleRequest` after unmarshaling the given `ExceptionInfoRequest` to populate the given `ExceptionInfoResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnExceptionInfoRequest func(*ExceptionInfoRequest, *ExceptionInfoResponse)error

// Called by `HandleRequest` after unmarshaling the given `GotoRequest` to populate the given `GotoResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnGotoRequest func(*GotoRequest, *GotoResponse)error

// Called by `HandleRequest` after unmarshaling the given `DisconnectRequest` to populate the given `DisconnectResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnDisconnectRequest func(*DisconnectRequest, *DisconnectResponse)error

// Called by `HandleRequest` after unmarshaling the given `StepInRequest` to populate the given `StepInResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnStepInRequest func(*StepInRequest, *StepInResponse)error

// Called by `HandleRequest` after unmarshaling the given `ThreadsRequest` to populate the given `ThreadsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnThreadsRequest func(*ThreadsRequest, *ThreadsResponse)error

// Called by `HandleRequest` after unmarshaling the given `GotoTargetsRequest` to populate the given `GotoTargetsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnGotoTargetsRequest func(*GotoTargetsRequest, *GotoTargetsResponse)error

// Called by `HandleRequest` after unmarshaling the given `SetFunctionBreakpointsRequest` to populate the given `SetFunctionBreakpointsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnSetFunctionBreakpointsRequest func(*SetFunctionBreakpointsRequest, *SetFunctionBreakpointsResponse)error

// Called by `HandleRequest` after unmarshaling the given `RestartRequest` to populate the given `RestartResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnRestartRequest func(*RestartRequest, *RestartResponse)error

// Called by `HandleRequest` after unmarshaling the given `PauseRequest` to populate the given `PauseResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnPauseRequest func(*PauseRequest, *PauseResponse)error

// Called by `HandleRequest` after unmarshaling the given `StepOutRequest` to populate the given `StepOutResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnStepOutRequest func(*StepOutRequest, *StepOutResponse)error

// Called by `HandleRequest` after unmarshaling the given `RestartFrameRequest` to populate the given `RestartFrameResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnRestartFrameRequest func(*RestartFrameRequest, *RestartFrameResponse)error

// Called by `HandleRequest` after unmarshaling the given `ReverseContinueRequest` to populate the given `ReverseContinueResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnReverseContinueRequest func(*ReverseContinueRequest, *ReverseContinueResponse)error

// Called by `HandleRequest` after unmarshaling the given `SetVariableRequest` to populate the given `SetVariableResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnSetVariableRequest func(*SetVariableRequest, *SetVariableResponse)error

// Called by `HandleRequest` after unmarshaling the given `StackTraceRequest` to populate the given `StackTraceResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnStackTraceRequest func(*StackTraceRequest, *StackTraceResponse)error

// Called by `HandleRequest` after unmarshaling the given `StepBackRequest` to populate the given `StepBackResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnStepBackRequest func(*StepBackRequest, *StepBackResponse)error

// Called by `HandleRequest` after unmarshaling the given `SourceRequest` to populate the given `SourceResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnSourceRequest func(*SourceRequest, *SourceResponse)error

// Called by `HandleRequest` after unmarshaling the given `SetBreakpointsRequest` to populate the given `SetBreakpointsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnSetBreakpointsRequest func(*SetBreakpointsRequest, *SetBreakpointsResponse)error

// Called by `HandleRequest` after unmarshaling the given `ScopesRequest` to populate the given `ScopesResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnScopesRequest func(*ScopesRequest, *ScopesResponse)error

// Called by `HandleRequest` after unmarshaling the given `LaunchRequest` to populate the given `LaunchResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnLaunchRequest func(*LaunchRequest, *LaunchResponse)error

// Called by `HandleRequest` after unmarshaling the given `ContinueRequest` to populate the given `ContinueResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnContinueRequest func(*ContinueRequest, *ContinueResponse)error

// Called by `HandleRequest` after unmarshaling the given `RunInTerminalRequest` to populate the given `RunInTerminalResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnRunInTerminalRequest func(*RunInTerminalRequest, *RunInTerminalResponse)error

// Called by `HandleRequest` after unmarshaling the given `AttachRequest` to populate the given `AttachResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnAttachRequest func(*AttachRequest, *AttachResponse)error

// Called by `HandleRequest` after unmarshaling the given `StepInTargetsRequest` to populate the given `StepInTargetsResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnStepInTargetsRequest func(*StepInTargetsRequest, *StepInTargetsResponse)error

// Called by `HandleRequest` after unmarshaling the given `VariablesRequest` to populate the given `VariablesResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnVariablesRequest func(*VariablesRequest, *VariablesResponse)error

// Called by `HandleRequest` after unmarshaling the given `ModulesRequest` to populate the given `ModulesResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
var OnModulesRequest func(*ModulesRequest, *ModulesResponse)error

// Called by `HandleRequest` after unmarshaling the given `EvaluateRequest` to populate the given `EvaluateResponse` that it'll then return a pointer to (in addition to this handler's returned `error`, if any).
