...
// Called by `HandleRequest` (after it unmarshaled the given `GotoTargetsRequest`) to further populate the given `GotoTargetsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnGotoTargetsRequest func(*GotoTargetsRequest, *GotoTargetsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `PauseRequest`) to further populate the given `PauseResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnPauseRequest func(*PauseRequest, *PauseResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `StackTraceRequest`) to further populate the given `StackTraceResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnStackTraceRequest func(*StackTraceRequest, *StackTraceResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `RestartRequest`) to further populate the given `RestartResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnRestartRequest func(*RestartRequest, *RestartResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `StepOutRequest`) to further populate the given `StepOutResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnStepOutRequest func(*StepOutRequest, *StepOutResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `StepInRequest`) to further populate the given `StepInResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnStepInRequest func(*StepInRequest, *StepInResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `VariablesRequest`) to further populate the given `VariablesResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnVariablesRequest func(*VariablesRequest, *VariablesResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ModulesRequest`) to further populate the given `ModulesResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnModulesRequest func(*ModulesRequest, *ModulesResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `CompletionsRequest`) to further populate the given `CompletionsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnCompletionsRequest func(*CompletionsRequest, *CompletionsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `DisconnectRequest`) to further populate the given `DisconnectResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnDisconnectRequest func(*DisconnectRequest, *DisconnectResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ThreadsRequest`) to further populate the given `ThreadsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnThreadsRequest func(*ThreadsRequest, *ThreadsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ContinueRequest`) to further populate the given `ContinueResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnContinueRequest func(*ContinueRequest, *ContinueResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `RestartFrameRequest`) to further populate the given `RestartFrameResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnRestartFrameRequest func(*RestartFrameRequest, *RestartFrameResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `StepInTargetsRequest`) to further populate the given `StepInTargetsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnStepInTargetsRequest func(*StepInTargetsRequest, *StepInTargetsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `SetFunctionBreakpointsRequest`) to further populate the given `SetFunctionBreakpointsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnSetFunctionBreakpointsRequest func(*SetFunctionBreakpointsRequest, *SetFunctionBreakpointsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `GotoRequest`) to further populate the given `GotoResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnGotoRequest func(*GotoRequest, *GotoResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `StepBackRequest`) to further populate the given `StepBackResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnStepBackRequest func(*StepBackRequest, *StepBackResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `NextRequest`) to further populate the given `NextResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnNextRequest func(*NextRequest, *NextResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ExceptionInfoRequest`) to further populate the given `ExceptionInfoResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnExceptionInfoRequest func(*ExceptionInfoRequest, *ExceptionInfoResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ReverseContinueRequest`) to further populate the given `ReverseContinueResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnReverseContinueRequest func(*ReverseContinueRequest, *ReverseContinueResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `AttachRequest`) to further populate the given `AttachResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnAttachRequest func(*AttachRequest, *AttachResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `RunInTerminalRequest`) to further populate the given `RunInTerminalResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnRunInTerminalRequest func(*RunInTerminalRequest, *RunInTerminalResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `SetVariableRequest`) to further populate the given `SetVariableResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnSetVariableRequest func(*SetVariableRequest, *SetVariableResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `SetBreakpointsRequest`) to further populate the given `SetBreakpointsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnSetBreakpointsRequest func(*SetBreakpointsRequest, *SetBreakpointsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ConfigurationDoneRequest`) to further populate the given `ConfigurationDoneResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnConfigurationDoneRequest func(*ConfigurationDoneRequest, *ConfigurationDoneResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `ScopesRequest`) to further populate the given `ScopesResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnScopesRequest func(*ScopesRequest, *ScopesResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `LaunchRequest`) to further populate the given `LaunchResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnLaunchRequest func(*LaunchRequest, *LaunchResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `InitializeRequest`) to further populate the given `InitializeResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnInitializeRequest func(*InitializeRequest, *InitializeResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `SourceRequest`) to further populate the given `SourceResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnSourceRequest func(*SourceRequest, *SourceResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `SetExceptionBreakpointsRequest`) to further populate the given `SetExceptionBreakpointsResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
var OnSetExceptionBreakpointsRequest func(*SetExceptionBreakpointsRequest, *SetExceptionBreakpointsResponse)error

// Called by `HandleRequest` (after it unmarshaled the given `EvaluateRequest`) to further populate the given `EvaluateResponse` before returning to its caller a pointer to it (in addition to this handler's returned `error`, if any).
