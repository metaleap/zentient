...
func (_ *zgo) Caps (cap string) (caps []*z.RespCmd) {
	if (!capsinit) {
		...
		capsext = []*z.RespCmd	{	{ Title: "guru",		Exists: devgo.Has_guru,			Hint: "golang.org/x/tools/cmd/guru",		More: "via <i>Code Intel Extras</i>: Callees, Callers, Callstack, Free Variables, Types of Errors, Points-To, Channel Peers" },
									{ Title: "godoc",		Exists: devgo.Has_godoc,		Hint: "golang.org/x/tools/godoc",			More: "via <i>Query Extras</i>. (If `godoc` not present, will fall back to `go doc`.)" },
									{ Title: "go run",		Exists: true,																More: "via <i>Query Extras</i>: attempt to evaluate the given expression in the current source context" },
								}
