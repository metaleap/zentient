...
func buildPkg(pkgimppath string, fallbackfilerelpath string, diags map[string]udev.SrcMsgs) bool {
	...
	for _, srcref := range msgs {
		if srcref.Msg != "too many errors" {
			fpath := srcref.Ref
			srcref.Ref = "go install " + pkgimppath
			srcref.Flag = z.DIAG_SEV_ERR
			if !ufs.FileExists(filepath.Join(srcDir, fpath)) {
				srcref.Msg = fpath + ": " + srcref.Msg
				fpath = fallbackfilerelpath
			}
...
func (_ *zgo) BuildFrom(filerelpaths []string) (freshdiags map[string]udev.SrcMsgs) {
	pkgimppaths := []string{}
	pkgimpimppaths := []string{}
	udevgo.GuruScopeExclPkgs = nil
	...
	refreshindirectdependants := func() {
		if asyncrebuilds := udevgo.AllFinalDependants(succeeded); len(asyncrebuilds) > 0 {
			defer refreshPkgs()
			laterebuilds.Lock()
			defer laterebuilds.Unlock()
			for _, pkgimppath := range asyncrebuilds {
				if !(uslice.StrHas(pkgimppaths, pkgimppath) || uslice.StrHas(pkgimpimppaths, pkgimppath) || uslice.StrHas(succeeded, pkgimppath)) {
					urun.CmdExec("go", "install", pkgimppath)
				}
			}
		}
	}
